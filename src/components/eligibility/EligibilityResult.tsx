...
const EligibilityResult = ({ result, onComplete, onGoBack, onSubmit }: EligibilityResultProps) => {
  ...
  return (
    <motion.div className="space-y-8 py-4 relative" variants={containerVariants} initial="hidden" animate="visible">
      ...
      <div className="absolute top-0 right-0 w-24 h-[2px] bg-gradient-to-l from-purple-300 to-transparent"></div>
      <div className="absolute bottom-0 left-0 w-24 h-[2px] bg-gradient-to-r from-purple-300 to-transparent"></div>
      ...
      >
        ...
        <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden">
          <div className={`absolute top-0 right-0 w-8 h-8 bg-purple-300 rotate-45 transform origin-bottom-left opacity-80`}></div>
        </div>
      ...
      {/* Only show eligibility score section if the application is eligible */}
      {result.eligible && (
        <motion.div variants={itemVariants} className="bg-gray-50 rounded-lg p-6 border border-gray-200 relative">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="text-sm flex items-center">
              <span className="flex items-center">
                <span className="w-3 h-3 bg-purple-300 rounded-full mr-2"></span>
          ...
          <div className="absolute bottom-3 right-3 flex items-center gap-1">
            <div className="w-1.5 h-1.5 rounded-full bg-purple-300 opacity-70"></div>
            <div className="w-1.5 h-1.5 rounded-full bg-purple-300 opacity-50"></div>
            <div className="w-1.5 h-1.5 rounded-full bg-purple-300 opacity-30"></div>
          </div>
        </motion.div>
      )}
      ...
      <motion.div variants={itemVariants} className="flex justify-between">
        <Button 
          type="button" 
          variant="outline" 
          className="border-finance-200 text-finance-700 hover:bg-finance-50 hover:border-purple-300"
          onClick={onGoBack}
        >
        ...
        {result.eligible && (
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button 
                type="button" 
                className="bg-finance-600 hover:bg-finance-700 text-white border-b-2 border-purple-300"
              >
            ...
            <AlertDialogContent className="border-purple-200">
              <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden">
                <div className="absolute top-0 right-0 w-8 h-8 bg-purple-300 rotate-45 transform origin-bottom-left opacity-50"></div>
              </div>
              <AlertDialogHeader>
                <AlertDialogTitle className="relative inline-block">
                  Confirm Submission
                  <span className="absolute bottom-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-purple-300 to-transparent"></span>
                </AlertDialogTitle>
                <AlertDialogDescription>
