...
const NavItem = ({ icon, label, active = false }: NavItemProps) => {
  return (
    <div className={`flex items-center gap-1.5 px-4 py-2 rounded-md cursor-pointer transition-all ${active ? 'bg-finance-100 text-finance-800 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}>
      {icon}
      <span>{label}</span>
    </div>
  );
